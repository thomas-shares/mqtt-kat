...
/**
 * PUBREC. The packet is identical in shape to the other three
...
public class MqttPubRec extends GenericMessage {
	...
	public static IPersistentMap decode(SelectionKey key, byte[] data, int protocolVersion)
			throws IOException {
		return MqttAck.decode(key, data, protocolVersion, intern("PUBREC"));
	}

	public static ByteBuffer encode(Map<Keyword, ?> message) {
		return MqttAck.encode(message, MESSAGE_PUBREC, 0);
	}
}
