...
/**
 * PUBREL. The packet is identical in shape to the other three
...
public class MqttPubRel extends GenericMessage {
	...
	public static IPersistentMap decode(SelectionKey key, byte[] data, int protocolVersion)
			throws IOException {
		return MqttAck.decode(key, data, protocolVersion, intern("PUBREL"));
	}

	public static ByteBuffer encode(Map<Keyword, ?> message) {
		return MqttAck.encode(message, MESSAGE_PUBREL, 0x02);
	}
}
